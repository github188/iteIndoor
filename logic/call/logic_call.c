...
static void * becall_proc(void *arg)
{
	...
	if (g_BeCallInfo.state == CALL_STATE_CALLING)
	{
		...
		if (g_BeCallInfo.LocalVideoSendrecv != _NONE)
		{
			...
			g_BeCallInfo.IsStartVideo = TRUE;
			...
			if (LWM_NONE != g_BeCallInfo.LeaveWordMode)
			{
				...
				{	 	 
					if (is_main_DeviceNo())
					{
						...
						// mody by chenbh 呼叫转留言提示音太快提示音播放不完整
						usleep(500*1000);         
					}
					else
					{
			...
			if (ECHO_OK == echo)
			{
				...
				switch (state)
				{
					...
					case TRC_RINGING:
						...
						{
							media_Ad_set_audio_PackMode(1);	// GM8126门前机应答是单包,实际是要组包发过去,否则会无声音
						}
						else
						{
