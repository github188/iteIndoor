...
{	
	...
	dprintf("ipmodule_jd_net_open JDType %x Index  %d data %d\n",JDType, Index, data[1]);
...
{	
	...
	dprintf("ipmodule_jd_net_close JDType %x Index %d\n",g_DevType, Index);
...
{	
	dprintf("ipmodule_jd_net_scene Scene %x\n",Scene);
...
uint32 ipmodule_jd_net_get_state(uint8 JDType, uint32 check, uint8 *Index, uint8 Count)
{
	...
	dprintf("data1 %x data2 %x num %x\n",data1,data2,num);
	
	SendSize += 8;

	dprintf("ipmodule_jd_net_get_state g_DevType %x %x %x %x %x %x %x %x %x\n",g_DevType,
...
uint32 ipmodule_jd_net_get_info(uint8 Type)
{
	...
	if (g_DevType > 5)
	{
		dprintf(">>>ipmodule_jd_net_get_info: devtype=%d error\n", g_DevType);
		return ECHO_ERROR;
	}
	...
	if (sendagain)
	{
		...
		if(EchoValue == ECHO_OK && ReciData != NULL && ReciSize > 0)
		{
			...
			dprintf("ipmodule_jd_net_get_info 1 count %d\n",count);
	...
	if (ret == FALSE)
	{
		dprintf(">>>ipmodule_jd_net_get_info: send_command error.\n");
		return ECHO_ERROR;
	}
	...
	if(EchoValue == ECHO_OK && ReciData != NULL && ReciSize > 0)
	{
		...
		dprintf("ipmodule_jd_net_get_info 2 count %d\n",g_count);		
		
		dprintf(">>>ipmodule_jd_net_get_info: Type=%d, g_count=%d\n", Type, g_count);
		if (g_count > MAX_AURINE_JD_DG_NUM || g_count < 0)
		{
			dprintf(">>>ipmodule_jd_net_get_info: count > 64, error.\n");
			return ECHO_ERROR;
		}		
		...
		if (Type == 5)
		{
			...
			#if 0
			dprintf("*********************************************\n");
			dprintf("g_scenelist->nCount %d\n",g_scenelist->nCount);

			dprintf("*********************************************\n");
			for(j = 0; j < count; j++)
			{
				dprintf("auDevInfo[%d].Index %d\n",j,auDevInfo[j].Index);
				dprintf("auDevInfo[%d].IsUsed %d\n",j,auDevInfo[j].IsUsed);
				dprintf("auDevInfo[%d].IsTune %d\n",j,auDevInfo[j].IsTune);
				dprintf("auDevInfo[%d].Addr %d\n",j,auDevInfo[j].Addr);
				dprintf("auDevInfo[%d].Name %s\n",j,auDevInfo[j].Name);
			}		
			dprintf("*********************************************\n");	
			#endif
			...
			for (i = 0; i < count; i++)
			{
				for(j = 0; j < g_jdList->nCount; j++)
				{
					if (j == (auDevInfo[i].Index-1) && auDevInfo[i].IsUsed)
					{
						...
						#if 0
						dprintf("g_jdList->pjd_dev_info[%d].JdType %d\n",j,g_jdList->pjd_dev_info[j].JdType);
						dprintf("g_jdList->pjd_dev_info[%d].Index %d\n",j,g_jdList->pjd_dev_info[j].Index);
						dprintf("g_jdList->pjd_dev_info[%d].IsTune %d\n",j,g_jdList->pjd_dev_info[j].IsTune);
						dprintf("g_jdList->pjd_dev_info[%d].Addr %d\n",j,g_jdList->pjd_dev_info[j].Address);
						dprintf("g_jdList->pjd_dev_info[%d].Name %s\n",j,g_jdList->pjd_dev_info[j].Name);
						dprintf("g_jdList->pjd_dev_info[%d].TextIDPos %x\n",j,g_jdList->pjd_dev_info[j].TextIDPos);
						dprintf("g_jdList->pjd_dev_info[%d].TextIDName %x\n",j,g_jdList->pjd_dev_info[j].TextIDName);
						#endif
					}
				}	
			}
